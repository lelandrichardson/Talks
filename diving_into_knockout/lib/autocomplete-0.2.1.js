...
var sanityChecks = function() {
  ...
  if ((typeof containerElOrId !== 'string' || containerElOrId === '') && !isElement(containerElOrId)) {
  ...
  if (!document.getElementById(containerElOrId) && !isElement(containerElOrId)) {
    window.alert('AutoComplete Error 1002: ' +
      'Element with id "' + containerElOrId + '" does not exist in the DOM.' +
      '\n\nExiting...');
    return false;
  }
...
var clickPage = function(e) {
  ...
  if(!($(e.target).parents().index(containerEl) >= 0)){
    hideOptions();
  }
};
...
var initDom = function() {
  ...
  containerEl = isElement(containerElOrId) ? $(containerElOrId) : $('#' + containerElOrId);
