...
// `prop` and `invoke` turn methods into functions:
trace(map(prop('length'), ['two', 'words']));
trace(map(invoke('toUpperCase'), ['two', 'words']));
// `lambda` can do this too:
trace(map('.length', ['two', 'words']));
trace(map('.toUpperCase()', ['two', 'words']));
// and `prop` and `lambda` can both implement projections:
var cities =  [['NYC', 'NY'], ['Boston', 'MA']];
trace(map('_[0]',cities));
trace(map(prop(0), cities));
trace(map(prop(1), cities));
trace(map('_.x', [{x:10, y:20}, {x:15, y:25}, {x:0, y:-5}]));
trace(map(prop('x'), [{x:10, y:20}, {x:15, y:25}, {x:0, y:-5}]));
trace(map(prop('y'), [{x:10, y:20}, {x:15, y:25}, {x:0, y:-5}]));
